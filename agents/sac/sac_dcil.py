...
from .sac_from_jaxrl_dcil import Batch, SACLearner
...
class SAC_DCIL(Agent, ABC):
	...
	def train_on_batch(self, batch):
		jaxrl_batch = Batch(
			...
			masks=squeeze(1 - batch["true_done"]),
			bonus_mask = squeeze(batch["bonus_mask"]), ## true if success + no relabelling
			# masks=squeeze(1 - batch["true_done"]),
			next_observations=batch["next_observation"],
			next_observations_bonus=batch["next_observation_bonus"] ## next observation + next goal
		)
